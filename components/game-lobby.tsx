...
import { createGame, findGameIdByCode, subscribeActiveGame } from "@/lib/game-store"
...
export default function GameLobby() {
  ...
  const [activeGameId, setActiveGameId] = useState<string | null | undefined>(undefined)

  useEffect(() => {
    if (!firebaseConfigured()) return
    return subscribeActiveGame(setActiveGameId, (error) => setMessage(`Could not check for an active game: ${error.message}`))
  }, [])
  ...
  return <div className="grid gap-6 lg:grid-cols-2">
    {activeGameId ? <section className="rounded-2xl border border-[#cbd5cc] bg-[#f3ecdc] p-6"><h1 className="text-3xl font-bold text-[#123a28]">Game Already In Progress</h1><p className="mt-3 leading-7 text-[#52615a]">This class has an active duplicate game. Ask the director for its 6-character join code.</p></section> : activeGameId === null ? <section className="rounded-2xl border border-[#cbd5cc] bg-[#f3ecdc] p-6"><h1 className="text-3xl font-bold text-[#123a28]">Start a Duplicate Game</h1><p className="mt-2 leading-7 text-[#52615a]">3 tables, 6 pairs, 12 boards. Pair names are optional.</p>
