...
export default function GameLobby() {
  ...
  return <div className="mx-auto max-w-2xl space-y-6">
    {activeGame ? <section className="rounded-2xl border border-[#cbd5cc] bg-[#f3ecdc] p-6"><h1 className="text-3xl font-bold text-[#123a28]">Returning to Your Game</h1><p className="mt-3 leading-7 text-[#52615a]">Restoring director controls...</p><button type="button" onClick={() => router.replace(`/play/${activeGame.gameId}`)} className="mt-5 min-h-12 rounded-xl bg-[#1d5138] px-5 font-bold text-white">Return to Director Game</button></section> : activeGame === null ? <section className="rounded-2xl border border-[#cbd5cc] bg-[#f3ecdc] p-6"><h1 className="text-3xl font-bold text-[#123a28]">Start Today&apos;s Game</h1><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => setMovement("mitchell")} className={`min-h-11 rounded-lg px-4 font-bold ${movement === "mitchell" ? "bg-[#1d5138] text-white" : "bg-white text-[#173c2a]"}`}>Mitchell</button><button type="button" onClick={() => setMovement("howell")} className={`min-h-11 rounded-lg px-4 font-bold ${movement === "howell" ? "bg-[#1d5138] text-white" : "bg-white text-[#173c2a]"}`}>Howell</button></div>
